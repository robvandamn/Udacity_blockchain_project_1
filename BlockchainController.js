...
class BlockchainController {
    ...
    // Endpoint that allows user to check blockchain validity (GET Endpoint)
    validateChain() {
        this.app.get("/validatechain", async (req, res) => {
            const error_array = await this.blockchain.validateChain();
            if(error_array){
                return res.status(200).json(error_array);
            } else {
                return res.status(500).send("An error happened!");
            }
        });
